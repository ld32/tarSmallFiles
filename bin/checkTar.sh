...
function archiveFiles() {
    ...
    if [ "$length" -gt "25" ]; then
        # Create a new file name by truncating the original name
        item="${item:0:25}"
    fi 
    ...
    tar --create --preserve-permissions --file "$tmp" -T $tmp.list || { echo Error: tar $1 | tee -a $logDir/tarError$2.txt; return; } 
    
    checkSum=$(md5sum "$tmp" | awk '{ print $1 }') || { echo Error: checksum $1 | tee -a  $logDir/tarError$2.txt; return; } 
...
if [[ "$action" == scan ]]; then 
    ...
    echo "export dFolderTmp=`mktemp -d`" >> $logDir/job.sh
    #echo "trap \"rm -fr \$dFolderTmp \" EXIT"  >> $logDir/job.sh
    
    echo "trap \"rm -r \$dFolderTmp \$logDir/exclusive 2>/dev/null; echo exiting and delete lock; \" EXIT" >> $logDir/job.sh
